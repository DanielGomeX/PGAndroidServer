		...
		<form  method="get">   
			<div class="row">
				<div class="col-lg-8 col-md-8 col-sm-7 col-xs-6">					
					<h4>{{ trans('labels.pedido') }}s {{ $deleted ? trans('labels.disable') : trans('labels.enable')}}</h4>
				</div>
		    ...
		    @foreach ($objetoList as $objeto)
				<tr>
			        ...
			        <td class="col-md-2 center">
			        	<a href="{{ route('pedido_edit', ['id' => $objeto->id])}}" class="btn btn-warning">
			        		<span class="glyphicon glyphicon-search" aria-hidden="true"></span>
			        	</a> 
			        	<form  class="hide" id="theForm_{{ $objeto->id }}" name="theForm_{{ $objeto->id }}" method="POST" action="{{ route('pedido_delete', ['id' => $objeto->id]) }}">
				    		{!! csrf_field() !!}
